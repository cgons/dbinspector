...
def test_populate_stations_on_conflict(dbsession, mock_station_resp):
    ...
    dbsession.add_all([
        m.Station(code="AC", name="Action GO"),
        m.Station(code="UN", name="Union GO"),
    ])
    dbsession.commit()
    assert dbsession.query(m.Station).count() == 2
...
def test_fetch_stations():
    with requests_mock.Mocker() as req_mock:
        mock_resp = {'GetDepartStationsResult': {'Stations': [{'Code': 'AC', 'Name': 'Acton GO'}], 'StatusCode': 200}}  # noqa
...
def test_check_eligibility(result_type, expected):
    with requests_mock.Mocker() as req_mock:
        mock_resp = {"CheckEligibleResult": {"Reason": "", "ResultType": result_type, "StatusCode": 200}}  # noqa
